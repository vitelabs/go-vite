...
func newAccountPoolBlock(block *common.AccountStateBlock,
...
type accountPoolBlock struct {
	forkBlock
	block    *common.AccountStateBlock
	recover  *recoverStat
...
func (accB *accountPoolBlock) ReferHashes() (keys []common.Hash, accounts []common.Hash, snapshot *common.Hash) {
	if accB.block.Source != nil {
		accounts = append(accounts, accB.block.Source.Hash)
	}
...
func (accB *accountPoolBlock) Height() uint64 {
	return accB.block.Height().Uint64()
}

func (accB *accountPoolBlock) Hash() common.Hash {
	return accB.block.Hash()
}

func (accB *accountPoolBlock) PrevHash() common.Hash {
	return accB.block.PrevHash()
}

func (accB *accountPoolBlock) Owner() *common.Address {
	signer := accB.block.Signer()
	return &signer
}
...
*/
func (accP *accountPool) pendingAccountTo(h *common.HashHeight, sHeight uint64) (*common.HashHeight, error) {
	accP.chainHeadMu.Lock()
	defer accP.chainHeadMu.Unlock()
	accP.chainTailMu.Lock()
	defer accP.chainTailMu.Unlock()

	targetChain := accP.findInTree(h.Hash, h.Height.Uint64())
	if targetChain != nil {
...
func (accP *accountPool) findInPool(hash common.Hash, height uint64) bool {
	accP.blockpool.pendingMu.Lock()
	defer accP.blockpool.pendingMu.Unlock()
	return accP.blockpool.containsHash(hash)
}

func (accP *accountPool) findInTree(hash common.Hash, height uint64) tree.Branch {
	return accP.chainpool.tree.FindBranch(height, hash)
}

func (accP *accountPool) findInTreeDisk(hash common.Hash, height uint64, disk bool) tree.Branch {
...
func (accP *accountPool) AddDirectBlocks(received *accountPoolBlock) error {
	...
	case verifier.PENDING:
		msg := fmt.Sprintf("db for directly adding account block[%s-%s-%d].", received.block.Signer(), received.Hash(), received.Height())
		return errors.New(msg)
	case verifier.FAIL:
		if stat.err != nil {
			return stat.err
		}
		return errors.Errorf("directly adding account block[%s-%s-%d] fail.", received.block.Signer(), received.Hash(), received.Height())
...
func (accP *accountPool) makePackage(q batch.Batch, info *offsetInfo, max uint64) (uint64, error) {
	...
	if info.offset == nil {
		tailHeight, tailHash := current.TailHH()
		info.offset = &common.HashHeight{Hash: tailHash, Height: common.Height(tailHeight)}
		info.quotaUnused = accP.rw.getQuotaUnused()
	} else {
		block := current.GetKnot(info.offset.Height.Uint64()+1, false)
		if block == nil || block.PrevHash() != info.offset.Hash {
			return uint64(0), ErrCurrentChainModify
		}
	}

	minH := info.offset.Height.Uint64() + 1
	...
	for i := minH; i <= headH; i++ {
		...
		info.offset.Height = common.Height(block.Height())
		info.quotaSub(block)
	}
...
func (accP *accountPool) tryInsertItems(p batch.Batch, items []batch.Item, latestSb *common.SnapshotBlock, version uint64) error {
	accP.chainTailMu.Lock()
	defer accP.chainTailMu.Unlock()
...
func (accP *accountPool) genForSnapshotContents(p batch.Batch, b *snapshotPoolBlock, k common.Address, v *common.HashHeight) (bool, *stack.Stack) {
	accP.chainTailMu.Lock()
	defer accP.chainTailMu.Unlock()
	acurr := accP.CurrentChain()
	tailHeight, _ := acurr.TailHH()
	targetHash := acurr.GetHash(v.Height.Uint64(), true)
	if targetHash == nil {
		return true, nil
	}
	...
	if v.Height.Uint64() > tailHeight {
		// account block is in pool.
		tmp := stack.New()
		for h := v.Height.Uint64(); h > tailHeight; h-- {

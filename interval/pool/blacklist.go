...
func (bl *blacklist) Add(key common.Hash) {
	bl.AddAddTimeout(key, bl.defaultTimeout)
}

func (bl *blacklist) AddAddTimeout(key common.Hash, duration time.Duration) {
...
func (bl *blacklist) Exists(key common.Hash) bool {
	value, ok := bl.cache.Get(key)
	if ok {
...
func (bl *blacklist) Remove(key common.Hash) {
	bl.cache.Remove(key)
}

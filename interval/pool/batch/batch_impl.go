...
type batchSnapshot struct {
	...
	version         uint64
	all             map[common.Hash]*ownerLevel
	ls              []Level
...
func (self *batchSnapshot) Exists(hash common.Hash) bool {
...
func NewBatch(snapshotF SnapshotExistsFunc, accountF AccountExistsFunc, version uint64, max int) Batch {
	...
	return &batchSnapshot{all: make(map[common.Hash]*ownerLevel), ls: tmpLs, snapshotExistsF: snapshotF, accountExistsF: accountF, maxLevel: max, current: -1, lastSnapshot: -1, version: version, id: id}
}
...
func (self *batchSnapshot) AddAItem(b Item, sHash *common.Hash) error {
	if b.Owner() == nil {
		return self.addSnapshotItem(b)
	} else {
...
func (self *batchSnapshot) addAccountItem(b Item, sHash *common.Hash) error {
	max := -1
	owner := b.Owner()
...
func (self *batchSnapshot) addToAll(keys []common.Hash, l *ownerLevel) {
	for _, v := range keys {
		self.all[v] = l
	}
}

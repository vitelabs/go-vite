...
func (sb *snapshotPoolBlock) ReferHashes() (keys []common.Hash, accounts []common.Hash, snapshot *common.Hash) {
...
func (sb *snapshotPoolBlock) Height() uint64 {
	return uint64(sb.block.Height())
}

func (sb *snapshotPoolBlock) Hash() common.Hash {
	return sb.block.Hash()
}

func (sb *snapshotPoolBlock) PrevHash() common.Hash {
	return sb.block.PrevHash()
}

func (sb *snapshotPoolBlock) Owner() *common.Address {
	return nil
}
...
func (sp *snapshotPool) snapshotFork(longest tree.Branch, current tree.Branch) error {
	...
	snapshots, accounts, e := sp.rw.delToHeight(keyPoint.block.Height().Uint64())
	if e != nil {
		return e
	}
...
func (sp *snapshotPool) snapshotInsertItems(p batch.Batch, items []batch.Item, version uint64) (map[common.Address][]commonBlock, batch.Item, error) {
...
func (sp *snapshotPool) snapshotWriteToChain(block *snapshotPoolBlock) (map[common.Address][]commonBlock, error) {
	height := block.Height()
	hash := block.Hash()
...
func (sp *snapshotPool) AddDirectBlock(block *snapshotPoolBlock) (map[common.Address][]commonBlock, error) {
	sp.chainHeadMu.Lock()
	defer sp.chainHeadMu.Unlock()
...
//	return blocks, nil
//}
func (sp *snapshotPool) fetchAccounts(accounts map[common.Address]*common.HashHeight, sHeight uint64, sHash common.Hash) {
...
func (sp *snapshotPool) genMaxAccounts(targetHeight uint64) (map[common.Address]*common.HashHeight, error) {
	result := make(map[common.Address]*common.HashHeight)
	cur := sp.CurrentChain()
	...
	for i := targetHeight; i > tailHeight; i-- {
		...
		for k, v := range block.SnapshotContent {
			if r := result[k]; r == nil {
				result[k] = &common.HashHeight{Hash: v.Hash, Height: v.Height}
			}
		}
	}
	return result, nil

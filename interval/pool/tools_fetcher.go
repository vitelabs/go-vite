...
type commonSyncer interface {
	fetch(hashHeight common.HashHeight, prevCnt uint64)
}
...
func (accSyn *accountSyncer) fetch(hashHeight common.HashHeight, prevCnt uint64) {
...
func (accSyn *accountSyncer) fetchBySnapshot(hashHeight common.HashHeight, account common.Address, prevCnt uint64, sHeight uint64, sHash common.Hash) {
	if hashHeight.Height > 0 {
...
func (accSyn *accountSyncer) fetchByHash(hash common.Hash, prevCnt uint64) {
...
func (sSync *snapshotSyncer) broadcastBlock(block *common.SnapshotBlock) {
	sSync.fetcher.BroadcastSnapshotBlock(block)
}

func (sSync *snapshotSyncer) fetch(hashHeight common.HashHeight, prevCnt uint64) {
	if hashHeight.Height > 0 {
		if prevCnt > 100 {
			prevCnt = 100
		}
...
func (sSync *snapshotSyncer) fetchByHash(hash common.Hash, prevCnt uint64) {

...
func (disk *branchChain) MatchHead(hash common.Hash) bool {
	_, h := disk.HeadHH()
	return hash == h
}
...
func (disk *branchChain) GetHashAndBranch(height uint64) (*common.Hash, tree.Branch) {
	hash := disk.GetHash(height, true)
	return hash, disk
}

func (disk *branchChain) TailHH() (uint64, common.Hash) {
	panic("not support")
}
...
func (disk *branchChain) GetHash(height uint64, flag bool) *common.Hash {
	return disk.rw.getHash(height)
}

func (disk *branchChain) ContainsKnot(height uint64, hash common.Hash, flag bool) bool {
...
func (disk *branchChain) HeadHH() (uint64, common.Hash) {
	h := disk.head
	if h == nil {
		head := disk.Head()

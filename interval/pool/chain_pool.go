...
func (cp *chainPool) forkFrom(forked tree.Branch, height uint64, hash common.Hash) (tree.Branch, error) {
	...
	for _, c := range chains {
		...
		for i := tH + 1; i <= snippet.headHeight; i++ {
			...
			if *b2 != sb.Hash() {
				...
				hash = b2
				// todo
				log.Debug(fmt.Sprintf("block[%s-%d] exists. del from tail.", sb.Hash(), sb.Height()))
...
func (cp *chainPool) insert(c tree.Branch, wrapper commonBlock) error {
	if cp.tree.Main().ID() == c.ID() {
		log.Debug(fmt.Sprintf("insert to current[%s]:[%s-%d]%s", c.ID(), wrapper.Hash(), wrapper.Height(), wrapper.Latency()))
	} else {
		log.Debug(fmt.Sprintf("insert to chain[%s]:[%s-%d]%s", c.ID(), wrapper.Hash(), wrapper.Height(), wrapper.Latency()))
	}
...
func (cp *chainPool) writeBlockToChain(block commonBlock) error {
	...
	if err != nil {
		// todo opt log
		log.Error(fmt.Sprintf("pool insert Chain fail. height:[%d], hash:[%s]", block.Height(), block.Hash()))
		return err
	}

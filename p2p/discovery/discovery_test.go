...
func mockAgent(port int) *agent {
	...
	a := &agent{
		...
		pool:    newWtPool(),
		log:     log15.New("module", "discv-test"),
	}

	handler := func(pkt *packet) {
		if pkt.code == pingCode {
			a.pong(pkt.from, pkt.hash)
		} else if pkt.code == pongCode {
			fmt.Println("receive pong from", pkt.from)
		}
	}
...
func TestPing_Pong_Local(t *testing.T) {
	port := 8483
	clt := mockAgent(port)
	...
	for i := 1; i < 100; i++ {
		...
		go func() {
			ch := make(chan bool, 1)
			clt.ping(svr.self.ID, svr.self.UDPAddr(), ch)
			done = <-ch
			wg.Done()
		}()
	}
...
func TestPing_Pong_Remote(t *testing.T) {
	pub, priv, err := ed25519.GenerateKey(nil)
	if err != nil {
		log.Fatal(err)
	}
	...
	nodes := make([]*Node, len(bootNodes))
	for i, str := range bootNodes {
		if n, err := ParseNode(str); err != nil {
			continue
		} else {
			nodes[i] = n
		}
...
func TestFind_Node_Remote(t *testing.T) {
	pub, priv, err := ed25519.GenerateKey(nil)
	if err != nil {
		log.Fatal(err)
	}
	...
	for _, n := range nodes {
		ch := make(chan []*Node, 1)
		d.findnode(n.ID, n.UDPAddr(), d.self.ID, maxNeighborsOneTrip, ch)

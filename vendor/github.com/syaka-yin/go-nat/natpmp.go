...
func (n *natpmpNAT) AddRandPortMapping(protocol string, internalPort int, description string, timeout time.Duration) (externalPort int, err error) {
...
func (n *natpmpNAT) AddPortMapping(protocol string, internalPort int, externalPort int, description string, timeout time.Duration) (err error) {
	timeoutInSeconds := int(timeout / time.Second)
	for i := 0; i < 3; i++ {
		_, err = n.c.AddPortMapping(protocol, internalPort, externalPort, timeoutInSeconds)
		if err == nil {

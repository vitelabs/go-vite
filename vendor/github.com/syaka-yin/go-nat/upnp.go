...
func (u *upnp_NAT) AddRandPortMapping(protocol string, internalPort int, description string, timeout time.Duration) (int, error) {
...
func (u *upnp_NAT) AddPortMapping(protocol string, internalPort int, externalPort int, description string, timeout time.Duration) (err error) {
	ip, err := u.GetInternalAddress()
	if err != nil {
		return err
	}

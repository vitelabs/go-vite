...
**`6`** This line `onMessage myListener(address dest) payable {` begins the declaration of a *message listener*, one of the most important function types in Solidity++. Message listeners will execute when the contract receives a cooresponding *message*, which can be sent from any Vite address, even from another contract.

For a message listener to execute, the message must indicate the message listener name (here it is `myListener`) as well as contain the appropriate arguments (`myListener` requires a single address `addr` as an argument).
...
**`7`** The code `dest.transfer(msg.tokenid, msg.amount);` initiates a transfer from the contract to the address stored in `dest` of all the tokens that were initially sent in the message that triggered `myListener`.

...
func (v *VmDebugApi) Init() (*VmAccountInfo, error) {
	...
	if prevBlock == nil {
		...
		if len(onroadList) > 0 && onroadList[0].FromAddress == types.AddressAssert && onroadList[0].Height == "2" {
...
func (v *VmDebugApi) CreateContract(param CreateContractParam) ([]*CreateContractResult, error) {
	...
	if param.AccountAddr == nil || len(v.accountMap[*param.AccountAddr]) == 0 {
		...
		testAccount = &VmAccountInfo{*param.AccountAddr, v.accountMap[*param.AccountAddr]}
	}

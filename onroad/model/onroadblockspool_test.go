...
func callTransfer(vite *VitePrepared, fromAddr, toAddr *types.Address,
	...
	_, fitestSnapshotBlockHash, err := generator.GetFitestGeneratorSnapshotHash(vite.chain, &im.AccountAddress, nil)
	if err != nil {
		return nil, err
	}
...
func createContract(vite *VitePrepared, addr *types.Address, addrPrivKey ed25519.PrivateKey, addrPubKey []byte, difficulty *big.Int) ([]*ledger.AccountBlock, error) {
	...
	data := util.GetCreateContractData(bytecode, types.DELEGATE_GID)
...
func receiveTransferSendBlocks(vite *VitePrepared, addr *types.Address) ([]*ledger.AccountBlock, error) {
	...
	for _, v := range sBlocks {
		...
		_, fitestSnapshotBlockHash, err := generator.GetFitestGeneratorSnapshotHash(vite.chain, &v.ToAddress, referredSnapshotHashList)
		if err != nil {
			return nil, err
		}

...
func (p *contractOnRoadPool) InsertAccountBlock(block *ledger.AccountBlock) error {
	...
	if block.IsSendBlock() {
		...
		if err := cc.(*callerCache).addTx(&onroad.caller, onroad.isCallerContract, &onroad.hashHeight, isWrite); err != nil {
		...
		if err := cc.(*callerCache).rmTx(&onroad.caller, onroad.isCallerContract, &onroad.hashHeight, isWrite); err != nil {
			mlog.Error(fmt.Sprintf("write block-r: %v %v %v fromHash=%v", block.AccountAddress, block.Height, block.Hash, block.FromBlockHash),
				"err", err)
			panic(ErrRmTxFailed)
		}
...
func (p *contractOnRoadPool) blockToOnRoad(block *ledger.AccountBlock) (*OnRoad, error) {
	...
	if block.IsSendBlock() {
		...
		or.hashHeight = ledger.HashHeight{
			Hash:   block.Hash,
			Height: block.Height,
		}
	} else {

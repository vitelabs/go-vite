...
const (
	txDataZeroGas               uint64 = 4     // Per byte of data attached to chain transaction that equals zero.
	txDataNonZeroGas            uint64 = 68    // Per byte of data attached to chain transaction that is not equal to zero.
	TxGas                       uint64 = 21000 // Per transaction not creating chain contract.
	PrecompiledContractsSendGas uint64 = 21068
	RefundGas                   uint64 = 21000
	txContractCreationGas       uint64 = 53000 // Per transaction that creates chain contract.
)

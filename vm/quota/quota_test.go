...
func TestCalcPoWDifficulty(t *testing.T) {
	...
	testCases := []struct {
		...
		{1, 0, 21000, types.NewQuota(0, 0, 0, 0, false, 0), big.NewInt(67108863), nil, "no_pledge_quota_before_hardfork"},
		{1, 0, 22000, types.NewQuota(0, 0, 0, 0, false, 0), big.NewInt(70689140), nil, "pledge_quota_not_enough_before_hardfork"},
		...
		{200, 0, 21000, types.NewQuota(0, 0, 0, 0, false, 0), big.NewInt(67108863), nil, "no_pledge_quota_after_hardfork"},
		{200, 0, 22000, types.NewQuota(0, 0, 0, 0, false, 0), big.NewInt(70689140), nil, "pledge_quota_not_enough_after_hardfork"},
		...
		{200, 74 * 51 * 21000, 21000, types.NewQuota(0, 0, 0, 0, false, 0), big.NewInt(67987247), nil, "no_pledge_quota_with_congestion_after_hardfork"},
		{200, 74 * 51 * 21000, 22000, types.NewQuota(0, 0, 0, 0, false, 0), big.NewInt(71614386), nil, "pledge_quota_not_enough_with_congestion_after_hardfork"},
...
func TestCalcQuotaV3(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		quotaTotal, pledgeQuota, quotaAddition, snapshotCurrentQuota, quotaAvg, _, _, err := calcQuotaV3(db, testCase.addr, getPledgeAmount(testCase.pledgeAmount), testCase.difficulty, testCase.sbHeight)
		if (err == nil && testCase.err != nil) || (err != nil && testCase.err == nil) || (err != nil && testCase.err != nil && err.Error() != testCase.err.Error()) {

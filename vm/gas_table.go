...
func gasCall(vm *VM, c *contract, stack *stack, mem *memory, memorySize uint64) (uint64, bool, error) {
	...
	quotaRatio, err := getQuotaRatioForRS(c.db, toAddress, c.sendBlock)
	if err != nil {
		return 0, true, err
	}
...
func getQuotaRatioForRS(db vm_db.VmDb, toAddr types.Address, sendBlock *ledger.AccountBlock) (uint8, error) {
	if !types.IsContractAddr(toAddr) {
		return util.CommonQuotaRatio, nil
	}
	...
	util.DealWithErr(err)
	if confirmSb != nil {
		meta, err := db.GetContractMetaInSnapshot(toAddr, confirmSb)
		util.DealWithErr(err)
		if meta == nil {
			return 0, util.ErrContractNotExists
		}

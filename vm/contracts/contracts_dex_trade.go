...
func (md *MethodDexTradeCancelOrderByHash) DoSend(db vm_db.VmDb, block *ledger.AccountBlock) (err error) {
	err = cabi.ABIDexTrade.UnpackMethod(new(dex.ParamDexCancelOrderByHash), cabi.MethodNameDexTradeCancelOrderByHash, block.Data)
	return
}
...
func (md MethodDexTradeCancelOrderByHash) DoReceive(db vm_db.VmDb, block *ledger.AccountBlock, sendBlock *ledger.AccountBlock, vm vmEnvironment) ([]*ledger.AccountBlock, error) {
	sendHashData := new(dex.ParamDexCancelOrderByHash)
	cabi.ABIDexTrade.UnpackMethod(sendHashData, cabi.MethodNameDexTradeCancelOrderByHash, sendBlock.Data)

	if orderId, ok := dex.GetOrderIdByHash(db, sendHashData.SendHash); !ok {

...
func GetVote(db StorageDatabase, gid types.Gid, addr types.Address) *types.VoteInfo {
	...
	data := db.GetStorageBySnapshotHash(&AddressVote, GetVoteKey(addr, gid), nil)
	if len(data) > 0 {
		nodeName := new(string)
...
func GetVoteList(db StorageDatabase, gid types.Gid, snapshotHash *types.Hash) []*types.VoteInfo {
	...
	if gid == types.DELEGATE_GID {
		iterator = db.NewStorageIteratorBySnapshotHash(&AddressVote, types.SNAPSHOT_GID.Bytes(), snapshotHash)
	} else {
		iterator = db.NewStorageIteratorBySnapshotHash(&AddressVote, gid.Bytes(), snapshotHash)
	}

...
func TestGenerator_GenerateWithOnroad(t *testing.T) {
	...
	_, fitestSnapshotBlockHash, err := GetFitestGeneratorSnapshotHash(v.chain, &fromBlock.ToAddress, referredSnapshotHashList)
	if err != nil {
...
func createRPCBlockCallPledgeContarct(vite *VitePrepared, addr *types.Address) error {
	...
	_, fitestSnapshotBlockHash, err := GetFitestGeneratorSnapshotHash(vite.chain, &im.AccountAddress, nil)
	if err != nil {
		return err
	}

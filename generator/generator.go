...
func NewGenerator(chain vm_db.Chain, addr types.Address, latestSnapshotBlockHash, prevBlockHash *types.Hash, states *util.GlobalStatus) (*Generator, error) {
	gen := &Generator{
...
func (gen *Generator) generateBlock(block *ledger.AccountBlock, fromBlock *ledger.AccountBlock, producer *types.Address, signFunc SignFunc) (result *GenResult, resultErr error) {
	...
	vmBlock, isRetry, err := gen.vm.Run(gen.vmDb, block, fromBlock, gen.states)
	if vmBlock != nil {

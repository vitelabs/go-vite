...
import (
	"github.com/vitelabs/go-vite/chain/state_bak/mvdb"
...
//)
//
//type undoOperations map[types.Address]map[uint64]uint64
//
type stateSnapshotManager struct {
	...
	//
	//	//undoOperations     map[types.Address]*undoOperation
	//
	//	//undoOperationsList map[types.Address]*list.List
	//	retainHeight       uint64
	//	undoOperationsList *list.List
	//
	//	pendingUndoLogs [][]byte
	//
...
func newStateSnapshotManager(chain Chain, mvDB *mvdb.MultiVersionDB) (*stateSnapshotManager, error) {
	...
	ssm := &stateSnapshotManager{
		...
		//		undoOperationsList: list.New(),
		//		pendingUndoLogs:    make(map[types.Address][][]byte),
...
//	latestSnapshotBlock := ssm.chain.GetLatestSnapshotBlock()
//	//snapshotBlocks , err := ssm.chain.GetSnapshotBlocks(&latestSnapshotBlock.Hash, false, 750)
//
...
//						snapshotHeight: latestSnapshotBlock.Height - currentConfirmedTimes,
//					})
//					undoOperations = make(map[uint64]uint64)
...
//
//	newUndoOperation := ssm.pendingUndoLogs[block.AccountAddress]
//	for keyId, valudId := range undoOperation {
//
//	}
//	ssm.pendingUndoLogs[block.AccountAddress] = append(ssm.pendingUndoLogs[block.AccountAddress], undoOperation)
//}
//
//func (ssm *stateSnapshotManager) Flush(snapshotHash *types.Hash) {
//
//	for addr, undoLog := range ssm.pendingUndoLogs {
//		undoOperations := ssm.mvDB.PosSeqUndoLogListToOperations(undoLogList),
//

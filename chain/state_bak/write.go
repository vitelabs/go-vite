...
func (sDB *StateDB) Write(block *vm_db.VmAccountBlock) error {
	...
	// write storage
	storageKeyList, storageValueList := sDB.prepareWriteStorage(address, unsavedStorage)
...
func (sDB *StateDB) prepareWriteStorage(addr *types.Address, unsavedStorage [][2][]byte) ([][]byte, [][]byte) {

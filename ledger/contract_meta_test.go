...
func TestContractMeta_Serialize(t *testing.T) {
	...
	if len(byteBuf) != LengthBeforeDexFork+1 {
		t.Fatal(fmt.Sprintf("must be equal %d, but is  %d", LengthBeforeDexFork+1, len(byteBuf)))
	}
	...
	if err := checkSame(cmNew2, cm.Serialize()[:LengthBeforeDexFork]); err != nil {
		t.Fatal(err)
	}
	...
	if len(byteBuf2) != LengthBeforeDexFork+1 {
		t.Fatal(fmt.Sprintf("must be equal %d, but is  %d", LengthBeforeDexFork+1, len(byteBuf2)))
	}

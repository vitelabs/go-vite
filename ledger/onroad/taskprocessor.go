...
func (tp *ContractTaskProcessor) processOneAddress(task *contractTask) (canContinue bool) {
	...
	// 2.Generator(vm)
	gen, err := generator.NewGenerator(tp.worker.manager.Chain(), tp.worker.manager.Consensus().SBPReader(), task.Addr, addrState.LatestSnapshotHash, addrState.LatestAccountHash)
	if err != nil {
